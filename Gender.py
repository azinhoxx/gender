...
def highlightFace (net, frame, conf_threshold=0.7):
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence>conf_threshold:
            ...
            faceBoxes.append([x1,y1,x2,y2]) # координаты ограничивающей рамки
...
# Откройте видеофайл, картинку или камеру
video=cv2.VideoCapture(args.image if args.image else 0)
padding=20
while cv2.waitKey(1)<0:
    hasFrame,frame=video.read() #Чтение рамки
    if not hasFrame:
        cv2.waitKey()
        break
    ...
    for faceBox in faceBoxes:
        #Анализирует лицо, обнаруженное в рамке
